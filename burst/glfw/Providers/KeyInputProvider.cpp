...
burst::glfw::KeyInputProvider::KeyInputProvider()
	: m_GlfwKeyEventSubscription(g_KeyEventNotifier.subscribe(this))
{}
...
void burst::glfw::KeyInputProvider::on_key_event(GLFWwindow *window,
												 int key,
												 int scancode,
												 int action,
												 int mods)
{
	try {
		if(action == GLFW_PRESS || action == GLFW_RELEASE) {
			...
			g_Keys[static_cast<burst::u32>(burst_key)] = action == GLFW_PRESS ? KeyState::Pressed
																			  : KeyState::Released;
		}
